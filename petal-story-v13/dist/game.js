import {GameModel,SKILLS,PLATFORMS,WORLD,FLOOR,xpNeeded,clamp} from './engine.js?v=6';
import {HeroAnimator,smoothSource,createHeroClips} from './hero-animation.js?v=12';
import {HERO_ASSETS} from './hero-assets.js?v=13';
import {ParallaxScene} from './parallax.js?v=9';
...
let app,world,parallax,fxLayer,hero,heroAnimator,heroFrames,monsterTexture,portraitCanvas,game,viewW=1200,viewH=720,camera=0,scale=1,uiTimer=0,lastSave=0,toastTimer=0,panelType='',wasPaused=false,soundEnabled=false,audioContext,ready=false;
...
async function init(){
  ...
  const [heroAtlas,objects,landscape,walkAtlas,slashAtlas,boltAtlas,rainAtlas,reactionAtlas]=await Promise.all([atlas('assets/hero.webp',4,2,[[80,39,352,503],[420,40,740,502],[788,40,1089,502],[1174,28,1490,441],[20,560,449,970],[457,521,755,981],[797,576,1108,968],[1115,740,1528,958]]),atlas('assets/objects.webp',2,2,[[122,132,509,505],[635,260,1240,480],[58,653,570,1182],[651,721,1221,1127]]),P.Assets.load(['assets/forest-far-v7.webp','assets/trees-v6.webp','assets/foliage-v6.webp']),sequenceAtlas(HERO_ASSETS.walk),sequenceAtlas(HERO_ASSETS.slash),sequenceAtlas(HERO_ASSETS.bolt),sequenceAtlas(HERO_ASSETS.rain),sequenceAtlas(HERO_ASSETS.reaction)]);
  for(const texture of Object.values(landscape))smoothSource(texture.source);
  heroFrames=heroAtlas.frames;monsterTexture=objects.frames[0];
  parallax=new ParallaxScene(P,{far:landscape['assets/forest-far-v7.webp'],trees:landscape['assets/trees-v6.webp'],foreground:landscape['assets/foliage-v6.webp']},SCENERY_ART);
  app.stage.addChild(parallax.back);
  world=new P.Container();app.stage.addChild(world);
  // Source grass contact line is y=305 inside the [635,260,1240,480] tile.
  // Align this line with collision y, not with the transparent texture edge.
  const shelf=(texture,x,y,w,h)=>addSprite(texture,x,y-h*45/220,w,h);
  const earth=await P.Assets.load('assets/ground-v11.webp');smoothSource(earth.source);
  const ground=createGround(P,world,earth,FLOOR);
  for(const platform of PLATFORMS.slice(1))shelf(objects.frames[1],platform.x,platform.y,platform.w,96);
  ...
  const idleAtlas=await sequenceAtlas(HERO_ASSETS.idle);
  ...
  app.stage.addChild(parallax.front);
  ...
  function resize(){const w=app.screen.width,h=app.screen.height;scale=h/720;viewH=720;viewW=w/scale;world.scale.set(scale);ground.resize(Math.max(WORLD,viewW),viewH);parallax.resize(viewW,scale,WORLD);}
  ...
  app.ticker.add(ticker=>{const dt=Math.min(ticker.deltaMS/1000,.04);game.update(dt,input);const p=game.player;
    camera=clamp(p.x-viewW*.38,0,Math.max(0,WORLD-viewW));world.x=-camera*scale;parallax.update(camera);
