...
export const MONSTER_FADE_IN = .65;
export const MONSTER_DEATH_DURATION = .96;
export const PLATFORMS = MAPS.meadow.platforms;
export const SKILLS = [
...
export class GameModel {
  constructor(save={},emit=()=>{}) {
    ...
    this.map=MAPS[save.mapId]||MAPS.meadow;this.portalCooldown=1;this.portalHeld=false;
    this.player.x=this.map.spawn;this.player.y=this.map.floor;
    this.populateMap();
  }
  populateMap(){this.monsters=this.map.spawns.map(([x,p])=>this.makeMonster(x,p));}
  get nearPortal(){const p=this.player;return !p.dead&&p.grounded&&Math.abs(p.x-this.map.portal.x)<70&&Math.abs(p.y-this.map.floor)<8;}
  travel(){if(this.paused||!this.nearPortal||this.portalCooldown>0)return false;if(this.onTravelRequest){this.onTravelRequest(this.map.portal.target);return true;}return this.completeTravel();}
  completeTravel(){const portal=this.map.portal;this.map=MAPS[portal.target];Object.assign(this.player,{x:portal.arrival,y:this.map.floor,vx:0,vy:0,grounded:true,action:0,actionKind:'idle',invuln:2});this.pendingCast=null;this.projectiles=[];this.portalCooldown=1.5;this.downHeld=false;this.populateMap();this.emit('map',{id:this.map.id});return true;}
  ...
  makeMonster(x,platform){const s=monsterStats(this.level);return {id:this.nextId++,x,y:this.map.platforms[platform].y,home:x,platform,hp:s.maxHp,maxHp:s.maxHp,level:this.level,dir:-1,hit:0,dead:0,deathElapsed:0,spawnAge:0,knockVx:0,stagger:0,phase:x*.013};}
  serialize(){return {version:2,mapId:this.map.id,level:this.level,xp:this.xp,stats:this.stats,skillLevels:this.skillLevels,kills:this.kills};}
  ...
  hurtMonster(m,damage,direction=Math.sign(m.x-this.player.x)||this.player.facing){if(m.dead)return;m.hp=Math.max(0,m.hp-damage);m.hit=.2;m.knockVx=(Math.sign(direction)||1)*440;m.stagger=.3;this.emit('hit',{x:m.x,y:m.y-60,damage});if(m.hp<=0){m.dead=4.5;m.deathElapsed=0;this.kills++;this.emit('kill',{x:m.x,y:m.y-30,xp:monsterStats(this.level).xp});this.gainXP(monsterStats(this.level).xp);}}
  moveKnockback(m,dt){const plat=this.map.platforms[m.platform];m.x=clamp(m.x+m.knockVx*dt,plat.x+30,plat.x+plat.w-30);m.knockVx*=Math.exp(-8*dt);m.stagger=Math.max(0,m.stagger-dt);if(!m.stagger)m.knockVx=0;}
  ...
  hurtPlayer(damage){const p=this.player;if(p.dead||p.invuln>0)return;p.hp=Math.max(0,p.hp-damage);p.invuln=1;
    const casting=p.action>0&&SKILLS.some(s=>s.animation===p.actionKind);
    if(!casting){p.action=.25;p.actionKind='hurt';p.actionSerial++;p.frame=6;}
    this.emit('hurt',{damage,x:p.x,y:p.y-110});
    if(p.hp===0){p.dead=3;p.frame=7;p.action=0;p.vx=0;this.pendingCast=null;this.emit('death',{x:p.x,y:p.y-55});}
  }
  respawn(){const p=this.player;Object.assign(p,{x:this.map.spawn,y:this.map.floor,vx:0,vy:0,hp:this.maxHp,mp:this.maxMp,dead:0,invuln:3,grounded:true,action:0,actionKind:'idle',frame:0});this.pendingCast=null;this.cooldowns=[0,0,0];this.projectiles=[];this.emit('respawn',{x:p.x,y:p.y-55});}
  update(dt,input={}) {
    ...
    const entering=!!(input.interact||input.touchJump),fresh=entering&&!this.portalHeld;this.portalHeld=entering;
    if(fresh&&this.travel()){input.touchJump=false;return;}
    ...
    const direction=(input.right?1:0)-(input.left?1:0);if(direction||input.jump||input.touchJump||input.down)this.active=true;
    p.vx=direction*265*(p.action>0?.25:1);if(direction&&p.action===0)p.facing=direction;
    const drop=input.down&&!this.downHeld&&p.grounded&&p.y<FLOOR-1;
    this.downHeld=!!input.down;
    if(drop){p.y+=4;p.vy=100;p.grounded=false;}
    else if((input.jump||input.touchJump)&&p.grounded){p.vy=-760;p.grounded=false;this.emit('jump',{});}
    input.touchJump=false;
    const prevY=p.y;p.vy+=1550*dt;p.x=clamp(p.x+p.vx*dt,45,this.map.width-45);p.y+=p.vy*dt;p.grounded=false;
    if(p.vy>=0)for(const platform of this.map.platforms){if(p.x+17>platform.x&&p.x-17<platform.x+platform.w&&prevY<=platform.y+1&&p.y>=platform.y){p.y=platform.y;p.vy=0;p.grounded=true;break;}}
    ...
    for(const m of this.monsters){
      if(m.stagger>0)this.moveKnockback(m,dt);
      if(m.dead){m.deathElapsed+=dt;m.dead-=dt;if(m.dead<=0){const replacement=this.makeMonster(m.home,m.platform);Object.assign(m,replacement);this.emit('spawn',{x:m.x,y:m.y});}continue;}
      m.spawnAge=Math.min(MONSTER_FADE_IN,m.spawnAge+dt);
      if(m.stagger>0){m.hit=Math.max(0,m.hit-dt);continue;}
      m.hit=Math.max(0,m.hit-dt);const plat=this.map.platforms[m.platform],near=this.active&&Math.abs(p.x-m.x)<420&&Math.abs(p.y-m.y)<80;
      ...
      if(this.active&&m.spawnAge>=MONSTER_FADE_IN&&Math.abs(p.x-m.x)<40&&Math.abs(p.y-m.y)<58)this.hurtPlayer(Math.max(3,monsterStats(this.level).attack-this.defense));
    }
    for(const projectile of this.projectiles){const prevX=projectile.x;projectile.x+=projectile.dir*690*dt;projectile.life-=dt;
      for(const m of this.monsters)if(!m.dead&&!projectile.hits.has(m.id)&&Math.abs(m.y-40-projectile.y)<65&&m.x>=Math.min(prevX,projectile.x)-35&&m.x<=Math.max(prevX,projectile.x)+35){projectile.hits.add(m.id);this.hurtMonster(m,projectile.damage,projectile.dir);}}
